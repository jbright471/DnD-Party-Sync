...
export default function DmDashboard({ 
    ...
    return (
        <div className="flex flex-col h-full gap-4 overflow-hidden relative text-white">
            ...
            {showNpcManager && <NPCManager onClose={() => setShowNpcManager(false)} />}
            {showLootManager && <LootManager party={party} onClose={() => setShowLootManager(false)} />}

            {/* Top Row: God-Eye View + World Panel */}
            <div className="flex-none grid grid-cols-12 gap-4">
                <section className="col-span-12 lg:col-span-9 bg-dnd-surface border border-dnd-border p-4 rounded-lg shadow-xl">
                    <div className="flex justify-between items-center mb-4 text-shadow-lg">
                        ...
                        <div className="flex items-center gap-4">
                            <button onClick={() => setShowLootManager(true)} className="text-[10px] font-bold text-dnd-gold hover:text-white uppercase tracking-widest bg-dnd-gold/10 px-3 py-1 rounded border border-dnd-gold/20 transition-all">💰 Loot</button>
                            <button onClick={() => setShowNpcManager(true)} className="text-[10px] font-bold text-dnd-gold hover:text-white uppercase tracking-widest bg-dnd-gold/10 px-3 py-1 rounded border border-dnd-gold/20 transition-all">👥 NPCs</button>
                            <button onClick={() => setShowBuffManager(true)} className="text-[10px] font-bold text-dnd-gold hover:text-white uppercase tracking-widest bg-dnd-gold/10 px-3 py-1 rounded border border-dnd-gold/20 transition-all">✨ Buffs</button>
                    ...
                    <div className="grid grid-cols-[repeat(auto-fill,minmax(240px,1fr))] gap-3">
                        {party.map(char => (
                            <div key={char.id} className="bg-dnd-navy/50 border border-dnd-border p-3 rounded-lg hover:border-dnd-gold/30 transition-all flex flex-col gap-2">
                                <div className="flex justify-between items-start">
                                    <div className="cursor-pointer overflow-hidden" onClick={() => onCharacterClick(char)}><span className="text-sm font-bold block truncate">{char.name}</span><span className="text-[9px] text-dnd-muted uppercase truncate">{(char.classes || []).map(c => c.name).join('/')}</span></div>
                                    <div className="text-right flex-none"><span className="text-xs font-bold text-dnd-gold block font-mono">AC {char.ac}</span></div>
                                </div>
                                <div className="w-full bg-black h-1.5 rounded-full overflow-hidden border border-white/5"><div className={`h-full transition-all duration-500 ${char.currentHp / char.maxHp < 0.3 ? 'bg-dnd-red' : 'bg-dnd-green'}`} style={{ width: `${(char.currentHp / char.maxHp) * 100}%` }}></div></div>
                                <div className="flex items-center justify-between gap-1 mt-1">
                                    <div className="flex gap-1">
                                        <button onClick={() => handleQuickHp(char.id, -5)} className="w-6 h-6 flex items-center justify-center bg-dnd-red/10 text-dnd-red border border-dnd-red/20 rounded hover:bg-dnd-red/20 text-[10px] font-bold">-5</button>
                                        <button onClick={() => handleQuickHp(char.id, 5)} className="w-6 h-6 flex items-center justify-center bg-dnd-green/10 text-dnd-green border border-dnd-green/20 rounded hover:bg-dnd-green/20 text-[10px] font-bold">+5</button>
                                    </div>
                                    <div className="flex flex-wrap gap-1 justify-end overflow-hidden max-h-6">
                                        {(char.conditions || []).map((c, i) => (<span key={i} onClick={() => handleToggleCondition(char.id, char.conditions, c)} className="px-1 py-0.5 rounded bg-dnd-red/20 border border-dnd-red/30 text-dnd-red text-[7px] font-bold uppercase cursor-pointer">{c}</span>))}
                                    </div>
                                </div>
                            </div>
                        ))}
            ...
            <div className="flex-1 grid grid-cols-12 gap-4 overflow-hidden">
                ...
                <div className="col-span-12 lg:col-span-6 flex flex-col gap-4 overflow-hidden">
                    ...
                    <div className="flex-1 relative flex flex-col shadow-inner"><MapViewer isGm={true} party={party} /></div>
                </div>
                ...
                <div className="col-span-12 lg:col-span-3 flex flex-col gap-4 overflow-hidden">
                    ...
                    <div className="flex-1 flex flex-col overflow-hidden bg-dnd-surface border border-dnd-border rounded-lg shadow-xl">
                        <div className="p-2 border-b border-dnd-border bg-dnd-navy/30 flex justify-between items-center"><h4 className="text-[10px] font-bold uppercase tracking-widest text-dnd-muted m-0 italic">📜 Chronicle</h4></div>
