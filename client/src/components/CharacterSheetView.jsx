...
export default function CharacterSheetView({ character }) {
    ...
    return (
        <div className="h-full w-full overflow-y-auto bg-gray-900 p-3 md:p-6 text-white custom-scrollbar">
            ...
            {/* Header / Info Block */}
            <div className="flex flex-col xl:flex-row justify-between gap-4 border-b border-dnd-border pb-4 mb-4">
                <div className="flex items-center gap-4">
                    <div className="w-16 h-16 rounded-md bg-gray-950 border-2 border-dnd-gold flex items-center justify-center text-3xl shadow-[0_0_10px_rgba(210,160,23,0.3)] shrink-0">
                        🛡️
                    </div>
                    <div>
                        <h2 className="fantasy-heading text-2xl m-0 leading-tight text-white">{character.name}</h2>
                        <p className="text-dnd-gold m-0 text-sm uppercase tracking-widest font-semibold truncate max-w-[250px]">
                            {(Array.isArray(character.classes) ? character.classes : []).map(c => `Lv${c.level} ${c.name}`).join(' / ')}
                        </p>
                    </div>
                </div>

                <div className="flex gap-2 lg:gap-3 flex-wrap xl:flex-nowrap">
                    <div className="bg-gray-950 border border-dnd-red/50 rounded p-2 text-center flex-1 min-w-[70px]">
                        <div className="text-[10px] text-dnd-muted uppercase font-bold tracking-wider mb-1">Prof Bonus</div>
                        <div className="text-lg font-bold text-dnd-gold">+{profBonus}</div>
                    </div>
                    <div className="bg-gray-950 border border-dnd-red/50 rounded p-2 text-center flex-1 min-w-[70px]">
                        <div className="text-[10px] text-dnd-muted uppercase font-bold tracking-wider mb-1">Armor Class</div>
                        <div className="text-lg font-bold text-white">{character.ac}</div>
                    </div>
                    <div className="bg-gray-950 border border-dnd-red/50 rounded p-2 text-center flex-1 min-w-[70px]">
                        <div className="text-[10px] text-dnd-muted uppercase font-bold tracking-wider mb-1">Initiative</div>
                        <div className="text-lg font-bold text-white">{initiative >= 0 ? `+${initiative}` : initiative}</div>
                    </div>
                    <div className="bg-gray-950 border border-dnd-red/50 rounded p-2 text-center flex-1 min-w-[70px]">
                        <div className="text-[10px] text-dnd-muted uppercase font-bold tracking-wider mb-1">Speed</div>
                        <div className="text-lg font-bold text-white">{speed}ft</div>
                    </div>
                    <div className="bg-gray-950 border border-dnd-red/50 rounded p-2 text-center flex-1 min-w-[90px]">
                        <div className="text-[10px] text-dnd-muted uppercase font-bold tracking-wider mb-1">Hit Points</div>
                        <div className="text-lg font-bold text-dnd-green">{character.currentHp} <span className="text-xs text-dnd-muted">/ {character.maxHp}</span></div>
                    </div>
                </div>
            </div>
